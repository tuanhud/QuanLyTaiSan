...
namespace QuanLyTaiSan.Libraries
{
    public class ServerTimeHelper
    {
        ...
        public static DateTime getNow()
        {
            ...
            {
                //sql server time
                using (OurDBContext tmp = new OurDBContext(Global.server_database.get_connection_string()))
                {
                    DateTime current = tmp.Database.SqlQuery<DateTime>("select GETDATE()").FirstOrDefault();
                    return current;
                }
            }
            catch (Exception ex)
